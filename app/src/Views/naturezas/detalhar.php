<?php
$data_criacao = DateTime::createFromFormat("Y-m-d H:i:s", $natureza->getDataCriacao());
if ($natureza->getDataEdicao() != null) {
    $data_edicao = DateTime::createFromFormat("Y-m-d H:i:s", $natureza->getDataEdicao());
...
<h1>Natureza: <?= $natureza->getNome() ?></h1>
<br>
<a class="btn btn-secondary" href="<?= $_ENV['BASE_URL'] ?>/naturezas">Voltar</a>
<a class="btn btn-secondary" href="<?= $_ENV['BASE_URL'] ?>/naturezas/atualizar/<?= $natureza->getId() ?>"><i class="fa-solid fa-pen-to-square"></i> Atualizar</a>

<form method="POST" action="">
    <input type="hidden" name="natureza_id" value="<?= $natureza->getId() ?>">
    <?php if ($natureza->isEnabled()): ?>
        <button class="btn btn-secondary" name="type" value="unable"
            onclick="return confirm('Realmente deseja inativar este natureza?')">
    ...
    <?php else: ?>
        ...
        <button class="btn btn-secondary" name="type" value="enable"
            onclick="return confirm('Realmente deseja ativar este natureza?')">
            <i class="fa-solid fa-box-open"></i> Ativar
        </button>
    <?php endif; ?>
</form>
